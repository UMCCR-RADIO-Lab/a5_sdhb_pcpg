...
a5th.summary <- read.delim("/g/data/pq08/projects/ppgl/a5/wgs/analysis/telomerehunter/combined/telomerehunter_a5_summary.tsv")
...
a5th.singletons <- read.delim("/g/data/pq08/projects/ppgl/a5/wgs/analysis/telomerehunter/combined/telomerehunter_a5_singletons.tsv")
...
a5th.top_tvr <- read.delim("/g/data/pq08/projects/ppgl/a5/wgs/analysis/telomerehunter/combined/telomerehunter_a5_tvr_top_contexts.tsv")

##################################
# LINX Telomere Insertion Counts #
##################################

a5th.telosv <- read.delim("/g/data/pq08/projects/ppgl/a5/wgs/analysis/telomerehunter/combined/telomere_rep_inserts_count.tsv")
...
p1 <- a5th.summary.rna %>% 
  inner_join(a5_anno %>% dplyr::select(A5_ID, `Patient ID`, TERT_ATRX_Mutation, TERT_ATRX_Mutation_Event, 
               ...
               mutate(isMet_didMet=paste(is_primary_or_met, tumour_metastasised, sep="-"), 
                      ...
                      c_circle_result = factor(as.character(c_circle_result), levels = c("No data", "Negative", "Positive"))) %>% 
               dplyr::rename(PID=A5_ID)) %>% 
  ...
  plot.list[[g]] <-  log2cpm  %>% 
    ...
    left_join(a5_anno %>%  
                dplyr::select(A5_ID, `Patient ID`, TERT_ATRX_Mutation, 
                              ...
                              Primary_Location_Simplified , differential_group_sampletype_strict )) %>% 
...
```{r plots_expression_vs_telcontent_muttype }  
  log2cpm %>% 
    ...
    ggplot(aes(x=as.numeric(telhunter_log2_telcontentratio), y= log2_cpm_z, color=TERT_ATRX_Mutation, shape=TERT_ATRX_Mutation_Event, size=Primary_Met)) + 
    geom_point() + 
    #geom_text(aes(label=A5_ID)) + 
...
plot.data <- a5_anno %>%
    mutate(c_circle_result = factor(as.character(c_circle_result), levels = c("No data", "Negative", "Positive")),
           telhunter_log2_telcontentratio=as.numeric(telhunter_log2_telcontentratio),
