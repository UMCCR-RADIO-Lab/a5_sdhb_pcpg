...
plot_data <- wts_tcga_flynn_a5_lcpm.batch_removed %>% 
  ...
  filter(hgnc_symbol %in% c("TH", "DDC","DBH","PNMT")) %>% #, "SLC6A2"
  ...
  inner_join(a5_anno %>% 
               filter(!Exclude_RNA) %>% 
               dplyr::select(A5_ID, Primary_Location_Simplified, Catecholamine_profile, Lifetime_multiple_primary_ppgl_timing),
             by=c("Sample"="A5_ID")) %>% 
  dplyr::rename("A5_ID"="Sample") %>% 
  mutate(hgnc_symbol=factor(hgnc_symbol, levels=rev(c("TH","DDC" , "DBH","PNMT", "SLC6A2")))) %>% 
