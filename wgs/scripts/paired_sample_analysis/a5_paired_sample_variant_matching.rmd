...
```{r annotate_pileups, eval=FALSE, include=TRUE}

use_pileups=FALSE

if(use_pileups)
  {
#VAF of variants required for variant to be considered detected in mpileup
pileup_vaf_threshold=0.1
       ...
       for (tumour in tumour_samples){
         match_table <- match_table %>% 
           mutate("{tumour}_Called" := ifelse(!is.na(!!sym(paste0(tumour,"_pileup_vaf"))) &
                                                !!sym(paste0(tumour,"_pileup_vaf")) > pileup_vaf_threshold, 
                                                            yes="Y", 
...
```
```{r write_composed_plots, eval=TRUE, include=FALSE}
