# repeat region counts - differential accessibility analysis 
rm(list=ls())
library(tidyverse)
library(patchwork)
library(limma)
...
rpt_counts <- read_csv("Data/repeat_analysis/counts/repeat_counts.csv")
rpt_counts <- rpt_counts %>%
  dplyr::rename(NAM018 = "Chromaffin_cell") %>%
  data.frame() %>% 
  column_to_rownames(var = "repeat_name") 
...
plot_cpm(genesymbol = "TAR1")
plot_cpm(genesymbol = "(TTAGGG)n")
plot_cpm(genesymbol = "LTR12D")
plot_cpm(genesymbol = "LTR12C")

plot_cpm(genesymbol = "LTR12") +
  plot_cpm(genesymbol = "MER51A") + 
  plot_cpm(genesymbol = "(TAGGGT)n") +
  plot_cpm(genesymbol = "(CTGAGTCC)n") +
plot_layout(ncol = 2)
